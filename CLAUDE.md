...
src/
  ...
  lib/
    ...
    groups.ts               # pure group helpers (GROUP_COLORS palette, groupRuns, bondRange, repairGroups)
  ...
  types/item.ts             # PackingItem { id, title, count, weight?, luggageId, order, lastModified, groupId?, groupColor? }
  types/luggage.ts          # Luggage { id, name, order, lastModified }
  features/packing/
    hooks/usePackingItems.ts # items+luggages load, addItem/updateItem/updateItems/removeItem/importItems/reorderItems/moveItem/addLuggage/updateLuggage
    components/PackingList.tsx, LuggageColumn.tsx, AddLuggageDialog.tsx, AddItemRow.tsx, ItemRow.tsx, GroupHandle.tsx, TotalsBar.tsx, ImportExportBar.tsx, ListHeader.tsx
...
- Item **groups** bundle adjacent rows inside one luggage. Invariants (enforced by `repairGroups` in `lib/groups.ts` after every mutation): all members share a `luggageId`, members are a single contiguous run by `order`, and a group with fewer than 2 members is dissolved. Group state is two optional fields on `PackingItem` (`groupId` + `groupColor`, a `GROUP_COLORS` key) — no groups collection; `ignoreUndefinedProperties` on the Firestore instance makes clearing them a plain `undefined`.
- The `GroupHandle` circle before each title drives the bond gesture: pointerdown (`stopPropagation`, so the row's dnd-kit listeners never fire) → drag up/down → the previewed range is anchor↔hover grown to fully absorb any group it touches (`bondRange`), shown as a dashed blinking connector → release commits (`updateItems`), release more than `BOND_CANCEL_MARGIN` outside the column, or Escape, cancels. Committing onto an existing group keeps that group's id and color; a fresh group gets a hue not already used in the column. Double-clicking a grouped circle drops that item and every item below it out of the group (leaving a lone member above dissolves that group too). Both are disabled while a sort other than `default` is active, since displayed order isn't stored order.
- Drag semantics: dragging a group's **first** row moves the whole block; any other row joins the group it is dropped inside, keeps membership when dropped on that group's edge, and is detached otherwise. `LuggageColumn` renders each multi-item run inside a tinted `color-mix` wrapper, but the `SortableContext` id list stays flat.
- `receipt.ts` shows grouping as a colored spine only (no labels/subtotals), and export/import round-trip it as an optional `group` **color key**; import rebuilds groups from consecutive entries sharing that key within one luggage.
- Undo is an in-memory stack only (`history` ref in `usePackingItems`, capped at `MAX_HISTORY` snapshots of `{ items, luggages }`, dropped on reload). Every mutation calls `pushHistory()` first; writes that are part of a larger action pass `{ skipHistory: true }` so one user action equals one undo step. `undo()` swaps state back and writes only the difference (`sameItem` skips no-op item writes, plus `deleteItem`/`deleteLuggage` for anything the snapshot doesn't have).
